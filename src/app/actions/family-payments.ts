          ...
          try {
            ...
            const res = await fetch(XENDIT_SESSION_URL, {
              ...
              body: JSON.stringify({
                ...
                description: `TOPSELL RUN 6K Family - ${existingRegistration?.total_participants || 0} peserta`,
                ...getFamilyReturnUrls(existingPayment.payment_reference),
              }),
            })
            ...
            if (res.ok) {
              ...
              if (newCheckoutUrl && newXenditSessionId) {
                await updateFamilyPayment(existingPayment.id, {
            ...
            console.error('Failed to generate new checkout URL for existing payment:', err)
          }
        }
