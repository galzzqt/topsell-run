...
import { createInvitationPayment, simulateInvitationPaymentSuccess, syncXenditInvitationPaymentStatus } from '@/app/actions/invitation-payments'
import { InvitationParticipant, InvitationPayment, TOPSELL_RUN_EVENT, priceForCategory } from '@/lib/types'
...
function DashboardContent() {
  ...
  const [receiptData, setReceiptData] = useState<{ payment: InvitationPayment; participants: InvitationParticipant[] } | null>(null)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (session.user) {
        ...
        try {
          const { payments } = useInvitationStore.getState()
          const pending = payments.filter((p) => p.status === 'pending' && p.payment_reference)
          if (pending.length > 0) {
            await Promise.all(pending.map((p) => syncXenditInvitationPaymentStatus(p.payment_reference)))
            await fetchInvitationData(true)
          }
        } catch (err) {
  ...
  useEffect(() => {
    ...
    if (paymentResult !== 'success') {
      router.replace('/invitation-dashboard')
      return
    }
    ...
    const poll = async () => {
      attempts += 1
      if (paymentRef) await syncXenditInvitationPaymentStatus(paymentRef)
      await fetchInvitationData(true)
      const { payments } = useInvitationStore.getState()
      ...
      if (!cancelled && attempts >= 12) router.replace('/invitation-dashboard')
    }
    const intervalId = setInterval(poll, 2000)
    poll()
    return () => { cancelled = true; clearInterval(intervalId) }
  }, [fetchInvitationData, hasCelebratedPayment, router, searchParams, user?.id])
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await createInvitationPayment()
      ...
      if ('freePaid' in res) {
        confetti({ particleCount: 160, spread: 80, origin: { y: 0.6 }, colors: ['#ff2a44', '#ff6a00', '#ffffff'] })
        if (user?.id) await fetchInvitationData()
  ...
  const handleSimulatePayment = async () => {
    ...
    try {
      const res = await simulateInvitationPaymentSuccess(checkoutPayload.paymentId)
  ...
  const refreshPaymentStatus = useCallback(async (silent = false) => {
    ...
    try {
      const res = await syncXenditInvitationPaymentStatus(checkoutPayload.reference)
  ...
  }, [checkoutPayload, fetchInvitationData, hasCelebratedPayment, user?.id])
  ...
  const unitPrice = activeReg && activeReg.total_participants > 0
    ...
    : priceForCategory(invitation?.category)
    ...
    <div className="min-h-screen bg-brand-dark flex flex-col text-foreground">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-6 w-full flex flex-col gap-6 relative z-10">
        ...
        <div className="bg-card-bg border border-card-border rounded-xl p-4 flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-sport-orange">Pembayaran Invitation</p>
      ...
      {receiptData && invitation && (
        <EReceiptModal
          ...
          payer={{ ...invitation, family_code: invitation.invitation_code } as unknown as Family}
          type="invitation"
        />
      )}
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3 border-y border-card-border py-4">
              {[
                ...
                { label: 'Nama', value: invitation?.name },
