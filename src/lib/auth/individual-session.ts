const INDIVIDUAL_COOKIE = 'topsell_individual_session'
...
export async function getIndividualSessionFromRequest(request: { cookies: { get: (name: string) => { value: string } | undefined } }) {
  const token = request.cookies.get(INDIVIDUAL_COOKIE)?.value
