...
import { UMKM_COOKIE, type UmkmSession } from './umkm-session'

function getUmkmSecret() {
  const secret = process.env.UMKM_SESSION_SECRET || process.env.SESSION_SECRET
  if (!secret && process.env.NODE_ENV === 'production') {
    throw new Error('UMKM_SESSION_SECRET or SESSION_SECRET is required in production')
  }
...
async function sign(value: string) {
  const key = await getCryptoKey(getUmkmSecret())
  const encoder = new TextEncoder()
...
type EncodedSession = UmkmSession & { issuedAt: number }
...
export async function createUmkmSession(session: UmkmSession) {
  ...
  cookieStore.set(UMKM_COOKIE, token, {
    httpOnly: true,
    sameSite: 'lax',
...
export async function clearUmkmSession() {
  const cookieStore = await cookies()
  const headersList = await headers()
  const isHttps = headersList.get('x-forwarded-proto') === 'https'

  cookieStore.set(UMKM_COOKIE, '', {
    httpOnly: true,
    sameSite: 'lax',
    secure: process.env.NODE_ENV === 'production' && isHttps,
    path: '/',
...
export async function getUmkmSession(): Promise<UmkmSession | null> {
  const cookieStore = await cookies()
  const token = cookieStore.get(UMKM_COOKIE)?.value
  if (!token) return null

  const [encodedPayload, signature] = token.split('.')
  if (!encodedPayload || !signature) return null
  if (!getUmkmSecret()) return null
