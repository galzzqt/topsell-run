...
import { INVITATION_COOKIE, type InvitationSession } from './invitation-session'

function getInvitationSecret() {
  const secret = process.env.INVITATION_SESSION_SECRET || process.env.SESSION_SECRET
  if (!secret && process.env.NODE_ENV === 'production') {
    throw new Error('INVITATION_SESSION_SECRET or SESSION_SECRET is required in production')
  }
...
async function sign(value: string) {
  const key = await getCryptoKey(getInvitationSecret())
  const encoder = new TextEncoder()
...
type EncodedSession = InvitationSession & { issuedAt: number }
...
export async function createInvitationSession(session: InvitationSession) {
...
export async function clearInvitationSession() {
  const cookieStore = await cookies()
  const headersList = await headers()
  const isHttps = headersList.get('x-forwarded-proto') === 'https'

  cookieStore.set(INVITATION_COOKIE, '', {
    httpOnly: true,
    sameSite: 'lax',
    secure: process.env.NODE_ENV === 'production' && isHttps,
    path: '/',
...
export async function getInvitationSession(): Promise<InvitationSession | null> {
  const cookieStore = await cookies()
  ...
  if (!getInvitationSecret()) return null
