const PACER_COOKIE = 'topsell_pacer_session'
...
export async function getPacerSessionFromRequest(request: { cookies: { get: (name: string) => { value: string } | undefined } }) {
  const token = request.cookies.get(PACER_COOKIE)?.value
