...
export async function sendFamilyRegistrationConfirmationWebhook(payload: {
  ...
  return postWebhook('registration', {
    ...
    participant_count: payload.participantCount,
    message: `Pendaftaran Family Package ${payload.familyName} untuk TOPSELL RUN 2026 sudah diterima dengan ${payload.participantCount} anggota keluarga. Silakan masuk ke dashboard dan lakukan pembayaran agar Race Pass dan QR racepack aktif.`,
  })
}
...
export async function sendFamilyRacepackWebhook(payload: {
  ...
  return postWebhook('racepack', {
    ...
    message: `Pembayaran Family Package ${payload.familyName} untuk TOPSELL RUN 2026 sudah diterima. QR Code pengambilan racepack untuk ${payload.participantCount} anggota keluarga sudah dikirim ke email ${payload.email}. Setiap file QR dinamai sesuai nama peserta. Silakan cek inbox atau folder spam/promosi.`,
  })
}
