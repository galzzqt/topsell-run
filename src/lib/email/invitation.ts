...
function renderInvitationEmail(
  ...
  template?: EmailTemplateSettings['invitation']
) {
  const title = kind === 'receipt' ? 'E-RECEIPT PEMBAYARAN' : 'PEMBAYARAN DITERIMA'
  const variables = { invitationName: escapeHtml(name), invitationCode: escapeHtml(code), participantCount: '1' }
  ...
  const intro = kind === 'receipt'
    ...
    : 'Pembayaran invitation untuk TOPSELL RUN 2026 sudah kami terima. Kode QR untuk pengambilan racepack akan dikirimkan maksimal H-5 sebelum tanggal pengambilan racepack.'
...
export async function sendInvitationReceiptEmail(registrationId: string) {
  ...
  if (participants.length === 0) return { error: 'No invitation participants found for this registration' }

  const payment = await findInvitationPaymentById(participants[0].registration_id!)
  if (!payment) return { error: 'Payment not found' }

  const invitation = await findInvitationById(participants[0].invitation_id)
  if (!invitation || !invitation.email) return { error: 'Invitation or email not found' }

  try {
    await createTransporter().sendMail({
      from: getSmtpConfig().from,
      to: invitation.email,
      subject: `E-Receipt Pembayaran - TOPSELL RUN 2026 (${invitation.invitation_code})`,
      html: renderInvitationEmail(invitation.name, invitation.invitation_code, payment, participants[0], 'receipt'),
    })
    return { success: true }
  } catch (error) {
    console.error('Failed to send invitation receipt email:', error)
    return { error: error instanceof Error ? error.message : 'Gagal mengirim e-receipt' }
  }
}

export async function sendInvitationRacepackEmailsForRegistration(registrationId: string) {
  if (!isEmailConfigured()) return { skipped: true, sent: 0, failed: 0 }

  const participants = await findPaidInvitationParticipantsForRacepackEmail(registrationId)
  if (participants.length === 0) return { skipped: false, sent: 0, failed: 0 }

  const invitation = participants[0].invitation
  const email = invitation?.email
  const participantIds = participants.map((p) => p.id)

  if (!email) {
    await updateInvitationParticipantIds(participantIds, { racepack_email_error: 'Email peserta belum tersedia.' })
    return { skipped: false, sent: 0, failed: participants.length }
  }

  let payment: InvitationPayment | null = null
  try {
    const payments = await findInvitationPaymentsByRegistrationIds([registrationId])
    payment = payments.find((p) => p.status === 'paid') || payments[0] || null
  } catch {
    // optional
  }
  if (!payment) {
    await updateInvitationParticipantIds(participantIds, { racepack_email_error: 'Data pembayaran belum tersedia.' })
    return { skipped: false, sent: 0, failed: participants.length }
  }

  const emailSettings = await getEmailTemplateSettings()
  const subject = applyEmailVariables(emailSettings.invitation.subject, {
    invitationName: invitation?.name || 'Peserta',
  ...
  try {
    await createTransporter().sendMail({
      ...
      to: email,
      subject,
      html: renderInvitationEmail(invitation?.name || 'Peserta', invitation?.invitation_code || '-', payment, participants[0], 'racepack', emailSettings.invitation),
    })
    await updateInvitationParticipantIds(participantIds, {
    ...
    await updateInvitationParticipantIds(participantIds, { racepack_email_error: message })
    return { skipped: false, sent: 0, failed: participants.length }
  }
}
