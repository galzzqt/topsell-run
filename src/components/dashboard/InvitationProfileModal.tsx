...
import { updateInvitationProfile } from '@/app/actions/invitation-profile'
...
export function InvitationProfileModal({ isOpen, onClose }: InvitationProfileModalProps) {
  const { user, invitation, fetchInvitationData } = useInvitationStore()
  ...
  } = useForm<InvitationProfileValues>({
    resolver: zodResolver(invitationProfileSchema),
    defaultValues: {
      full_name: '',
      community_name: '',
      phone: '',
  ...
  }, [invitation, isOpen, reset])

  const onSubmit = async (values: InvitationProfileValues) => {
    const result = await updateInvitationProfile(values)
    if (result.error) {
      alert(result.error)
      return
    }
