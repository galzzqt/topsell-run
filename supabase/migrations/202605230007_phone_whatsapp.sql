-- Phone login and WhatsApp notification support.
...
alter table public.participants
  add column if not exists racepack_email_sent_at timestamptz,
  add column if not exists racepack_email_error text,
  add column if not exists racepack_whatsapp_sent_at timestamptz,
  add column if not exists racepack_whatsapp_error text;

create unique index if not exists communities_phone_unique_idx on public.communities(phone);
